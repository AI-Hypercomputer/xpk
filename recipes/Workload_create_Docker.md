# Workload create
Submits a basic workload to the cluster using Docker to build the container image.

# Running the command
```shell #golden
CRANE_WORKLOADS_ENABLED=False xpk workload create --project=golden-project --zone=us-central1-a --cluster=golden-cluster --workload=golden-workload --command "bash hello" --tpu-type=v5p-8 --num-slices=1 --script-dir=/tmp
```
<!--
$ CRANE_WORKLOADS_ENABLED=False xpk workload create --project=golden-project --zone=us-central1-a --cluster=golden-cluster --workload=golden-workload --command "bash hello" --tpu-type=v5p-8 --num-slices=1 --script-dir=/tmp
