...
func shouldFinalize(wl *kueue.Workload) bool {
...
func (r *WorkloadReconciler) finalize(ctx context.Context, wl *kueue.Workload) error {
	// No need to finalize if it is already finalized.
	...
	terminated, err := r.podsGracefullyTerminated(ctx, wl)
...
func (r *WorkloadReconciler) podsGracefullyTerminated(ctx context.Context, wl *kueue.Workload) (bool, error) {
	// For now, we only support JobSets.
	if isJobSetOwner(wl) {
		return r.jobSetPodsGracefullyTerminated(ctx, wl)
	}
	// Finalize Workloads that have no owner or have unsupported owner types.
	return true, nil
}

func (r *WorkloadReconciler) jobSetPodsGracefullyTerminated(ctx context.Context, wl *kueue.Workload) (bool, error) {
	...
	log.V(5).Info("Pods are still terminating – skipping finalization for now")
...
func shouldHandleWorkload(wl *kueue.Workload) bool {
	...
	return controllerutil.ContainsFinalizer(wl, CleanupSliceFinalizerName) || !shouldFinalize(wl)
}
