...
var _ = ginkgo.Describe("JobSet", func() {
	...
	ginkgo.When("Creating a JobSet", func() {
		...
		ginkgo.It("should create multiple Slices", func() {
			...
			ginkgo.By("Adding Forming condition for Slice 2", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					g.Expect(k8sClient.Get(ctx, sliceKey2, createdSlice2)).To(gomega.Succeed())
			...
			ginkgo.By("Adding Ready condition for Slice 1", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					g.Expect(k8sClient.Get(ctx, sliceKey1, createdSlice1)).To(gomega.Succeed())
					meta.SetStatusCondition(&createdSlice1.Status.Conditions, metav1.Condition{
						Type:    string(slice.Forming),
			...
			ginkgo.By("Adding Ready condition for Slice 2", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					g.Expect(k8sClient.Get(ctx, sliceKey2, createdSlice2)).To(gomega.Succeed())
					meta.SetStatusCondition(&createdSlice2.Status.Conditions, metav1.Condition{
						Type:    string(slice.Forming),
		...
		ginkgo.It("should handle two JobSets with the same name in different namespaces", func() {
			// Create two distinct namespaces for this test case
			ns1 := testing.MakeNamespaceWithGenerateName("e2e-jobset-ns1-")
			ns2 := testing.MakeNamespaceWithGenerateName("e2e-jobset-ns2-")
			utils.MustCreate(ctx, k8sClient, ns1)
			utils.MustCreate(ctx, k8sClient, ns2)

			// Defer cleanup for these new namespaces
			ginkgo.DeferCleanup(func() {
				gomega.Expect(utils.DeleteNamespace(ctx, k8sClient, ns1)).To(gomega.Succeed())
				gomega.Expect(utils.DeleteNamespace(ctx, k8sClient, ns2)).To(gomega.Succeed())
			})

			// Create local queues for each namespace, referencing the shared ClusterQueue
			lq1 := testing.MakeLocalQueue("lq-ns1", ns1.Name).ClusterQueue(cq.Name).Obj()
			lq2 := testing.MakeLocalQueue("lq-ns2", ns2.Name).ClusterQueue(cq.Name).Obj()
			utils.MustCreate(ctx, k8sClient, lq1)
			utils.MustCreate(ctx, k8sClient, lq2)
			ginkgo.DeferCleanup(func() {
				utils.ExpectObjectToBeDeleted(ctx, k8sClient, lq1, true)
				utils.ExpectObjectToBeDeleted(ctx, k8sClient, lq2, true)
			})

			const commonJobSetName = "jobset-common"
			tpuTopology := "4x4x4"
			tpuRequests := "4"
			parallelism := int32(16)
			replicas := int32(1)
			wantSliceSize := int32(16)

			jobSet1 := testingjobsjobset.MakeJobSet(commonJobSetName, ns1.Name).
				Queue(lq1.Name).
				ReplicatedJobs(
					testingjobsjobset.ReplicatedJobRequirements{
						Name:        "rj1",
				...
				RequestAndLimit("rj1", extraResource, tpuRequests).
				Obj()

			jobSet2 := testingjobsjobset.MakeJobSet(commonJobSetName, ns2.Name).
				Queue(lq2.Name).
				ReplicatedJobs(
					testingjobsjobset.ReplicatedJobRequirements{
						Name:        "rj1",
			...
			ginkgo.By("Creating JobSet 1", func() {
				utils.MustCreate(ctx, k8sClient, jobSet1)
			})
			ginkgo.By("Creating JobSet 2", func() {
				utils.MustCreate(ctx, k8sClient, jobSet2)
			})

			createdJobSet1 := &jobset.JobSet{}
			createdJobSet2 := &jobset.JobSet{}

			ginkgo.By("Checking that JobSet 1 is created with annotations/selectors", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(jobSet1), createdJobSet1)).To(gomega.Succeed())
					for _, replicatedJob := range createdJobSet1.Spec.ReplicatedJobs {
						annotations := replicatedJob.Template.Spec.Template.Annotations
						g.Expect(annotations["kueue.x-k8s.io/podset-required-topology"]).Should(gomega.Equal("cloud.google.com/gce-topology-block"))
						g.Expect(annotations["kueue.x-k8s.io/podset-slice-required-topology"]).Should(gomega.Equal("cloud.google.com/gke-tpu-slice-4x4x4-id"))
						g.Expect(annotations["kueue.x-k8s.io/podset-slice-size"]).Should(gomega.Equal(fmt.Sprint(wantSliceSize)))
						g.Expect(replicatedJob.Template.Spec.Template.Spec.NodeSelector["cloud.google.com/gke-tpu-slice-4x4x4-health"]).Should(gomega.Equal("true"))
					}
				}, utils.Timeout, utils.Interval).Should(gomega.Succeed())
			})

			ginkgo.By("Checking that JobSet 2 is created with annotations/selectors", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					g.Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(jobSet2), createdJobSet2)).To(gomega.Succeed())
					for _, replicatedJob := range createdJobSet2.Spec.ReplicatedJobs {
						annotations := replicatedJob.Template.Spec.Template.Annotations
						g.Expect(annotations["kueue.x-k8s.io/podset-required-topology"]).Should(gomega.Equal("cloud.google.com/gce-topology-block"))
						g.Expect(annotations["kueue.x-k8s.io/podset-slice-required-topology"]).Should(gomega.Equal("cloud.google.com/gke-tpu-slice-4x4x4-id"))
						g.Expect(annotations["kueue.x-k8s.io/podset-slice-size"]).Should(gomega.Equal(fmt.Sprint(wantSliceSize)))
			...
			wlKey1 := types.NamespacedName{
				Name:      jobsetcontroller.GetWorkloadNameForJobSet(jobSet1.Name, jobSet1.UID),
				Namespace: ns1.Name,
			}
			createdWorkload2 := &kueue.Workload{}
			wlKey2 := types.NamespacedName{
				Name:      jobsetcontroller.GetWorkloadNameForJobSet(jobSet2.Name, jobSet2.UID),
				Namespace: ns2.Name,
			}

			ginkgo.By("Waiting for Admission of Workload 1", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					g.Expect(k8sClient.Get(ctx, wlKey1, createdWorkload1)).Should(gomega.Succeed())
					g.Expect(createdWorkload1.Status.Admission).ShouldNot(gomega.BeNil())
				}, utils.Timeout, utils.Interval).Should(gomega.Succeed())
			})

			ginkgo.By("Waiting for Admission of Workload 2", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					g.Expect(k8sClient.Get(ctx, wlKey2, createdWorkload2)).Should(gomega.Succeed())
					g.Expect(createdWorkload2.Status.Admission).ShouldNot(gomega.BeNil())
				}, utils.Timeout, utils.Interval).Should(gomega.Succeed())
			})

			createdSlice1 := &slice.Slice{}
			sliceKey1 := core.SliceKeyFromWorkload(createdWorkload1, "rj1")
			createdSlice2 := &slice.Slice{}
			sliceKey2 := core.SliceKeyFromWorkload(createdWorkload2, "rj1")

			var slice1UID types.UID

			ginkgo.By("Checking that Slice 1 is created and setting to Ready", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					g.Expect(k8sClient.Get(ctx, sliceKey1, createdSlice1)).To(gomega.Succeed())
					slice1UID = createdSlice1.GetUID()
					g.Expect(createdSlice1.Spec.Topology).To(gomega.Equal(tpuTopology))
					g.Expect(createdSlice1.Spec.Type).To(gomega.Equal("tpu-v7x"))
					meta.SetStatusCondition(&createdSlice1.Status.Conditions, metav1.Condition{Type: string(slice.Ready), Status: metav1.ConditionTrue, Reason: "TestReady", Message: "Slice is ready"})
					g.Expect(k8sClient.Status().Update(ctx, createdSlice1)).To(gomega.Succeed())
				}, utils.Timeout, utils.Interval).Should(gomega.Succeed())
			})

			ginkgo.By("Checking that Slice 2 is created and setting to Ready", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					g.Expect(k8sClient.Get(ctx, sliceKey2, createdSlice2)).To(gomega.Succeed())
					g.Expect(createdSlice2.GetUID()).ShouldNot(gomega.Equal(slice1UID))
					g.Expect(createdSlice2.Spec.Topology).To(gomega.Equal(tpuTopology))
					g.Expect(createdSlice2.Spec.Type).To(gomega.Equal("tpu-v7x"))
					meta.SetStatusCondition(&createdSlice2.Status.Conditions, metav1.Condition{Type: string(slice.Ready), Status: metav1.ConditionTrue, Reason: "TestReady", Message: "Slice is ready"})
					g.Expect(k8sClient.Status().Update(ctx, createdSlice2)).To(gomega.Succeed())
				}, utils.Timeout, utils.Interval).Should(gomega.Succeed())
			})

			ginkgo.By("Checking that Workload 1 is admitted and admission check status is ready", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					g.Expect(k8sClient.Get(ctx, wlKey1, createdWorkload1)).Should(gomega.Succeed())
					g.Expect(workload.IsAdmitted(createdWorkload1)).Should(gomega.BeTrue())
					g.Expect(createdWorkload1.Status.AdmissionChecks).Should(gomega.BeComparableTo([]kueue.AdmissionCheckState{{
						Name:    kueue.AdmissionCheckReference(ac.Name),
						State:   kueue.CheckStateReady,
						Message: `Slices are in states: 1 Ready`,
					}}, cmpopts.IgnoreFields(kueue.AdmissionCheckState{}, "LastTransitionTime", "PodSetUpdates")))
			...
			ginkgo.By("Checking that Workload 2 is admitted and admission check status is ready", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					g.Expect(k8sClient.Get(ctx, wlKey2, createdWorkload2)).Should(gomega.Succeed())
					g.Expect(workload.IsAdmitted(createdWorkload2)).Should(gomega.BeTrue())
					g.Expect(createdWorkload2.Status.AdmissionChecks).Should(gomega.BeComparableTo([]kueue.AdmissionCheckState{{
						Name:    kueue.AdmissionCheckReference(ac.Name),
						State:   kueue.CheckStateReady,
						Message: `Slices are in states: 1 Ready`,
					}}, cmpopts.IgnoreFields(kueue.AdmissionCheckState{}, "LastTransitionTime", "PodSetUpdates")))
			...
			ginkgo.By("Deleting JobSet 1", func() {
				utils.ExpectObjectToBeDeleted(ctx, k8sClient, jobSet1, true)
			})
			ginkgo.By("Deleting JobSet 2", func() {
				utils.ExpectObjectToBeDeleted(ctx, k8sClient, jobSet2, true)
			})

			ginkgo.By("Checking that Slice 1 is deleted", func() {
				utils.ExpectObjectToBeDeleted(ctx, k8sClient, createdSlice1, false)
			})
			ginkgo.By("Checking that Slice 2 is deleted", func() {
				utils.ExpectObjectToBeDeleted(ctx, k8sClient, createdSlice2, false)
			})

			ginkgo.By("Checking that Workload 1 is deleted", func() {
				utils.ExpectObjectToBeDeleted(ctx, k8sClient, createdWorkload1, false)
			})
			ginkgo.By("Checking that Workload 2 is deleted", func() {
				utils.ExpectObjectToBeDeleted(ctx, k8sClient, createdWorkload2, false)
			})
		})
	})
})
