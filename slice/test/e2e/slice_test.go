		...
		ginkgo.It("should create Slice based on created Workload", func() {
			...
			ginkgo.By("Waiting for admission of workload and verify TopologyAssignment", func() {
				gomega.Eventually(func(g gomega.Gomega) {
					...
					g.Expect(createdWorkload.Status.Admission.PodSetAssignments).Should(gomega.HaveLen(2))
					g.Expect(createdWorkload.Status.Admission.PodSetAssignments[0].TopologyAssignment).Should(gomega.BeComparableTo(
					...
					g.Expect(createdWorkload.Status.Admission.PodSetAssignments[1].TopologyAssignment).Should(gomega.BeComparableTo(
						&kueue.TopologyAssignment{
							Levels: []string{corev1.LabelHostname},
							Domains: []kueue.TopologyDomainAssignment{{
								Count:  1,
