...
var _ = ginkgo.Describe("Subslicing", func() {
	...
	ginkgo.Describe("Multiple Replicas", func() {
		ginkgo.It("should create Slices for 2 replicas of 2x2x1 topology", func() {
			...
			ginkgo.By("Temporarily modify node capacities to prevent Kueue from putting both replicas in the same node", func() {
				nodes := &corev1.NodeList{}
				gomega.Expect(k8sClient.List(ctx, nodes)).To(gomega.Succeed())
				for _, node := range nodes.Items {
					gomega.Eventually(func(g gomega.Gomega) {
						n := &corev1.Node{}
						g.Expect(k8sClient.Get(ctx, client.ObjectKey{Name: node.Name}, n)).To(gomega.Succeed())
						err := utilclient.PatchStatus(ctx, k8sClient, n, func() (bool, error) {
							n.Status.Capacity[core.TPUResourceName] = resource.MustParse("4")
							n.Status.Allocatable[core.TPUResourceName] = resource.MustParse("4")
							return true, nil
						})
			...
			ginkgo.DeferCleanup(func() {
				...
				for _, node := range nodes.Items {
					gomega.Eventually(func(g gomega.Gomega) {
						...
						partitionID := n.Labels["cloud.google.com/gke-tpu-partition-2x2x1-id"]
						tpuCapacity := utils.GetTPUCapacityForSubslicePartition(partitionID)

						err := utilclient.PatchStatus(ctx, k8sClient, n, func() (bool, error) {
							n.Status.Capacity[core.TPUResourceName] = resource.MustParse(tpuCapacity)
							n.Status.Allocatable[core.TPUResourceName] = resource.MustParse(tpuCapacity)
							return true, nil
						})
						g.Expect(err).NotTo(gomega.HaveOccurred())
					}, utils.Timeout, utils.Interval).Should(gomega.Succeed())
			...
			jobSet := testingjobsjobset.MakeJobSet("jobset-multi-rjs", ns.Name).
				...
				ReplicatedJobs(
					testingjobsjobset.ReplicatedJobRequirements{
						...
						NodeSelector: map[string]string{
							"cloud.google.com/gke-tpu-accelerator":        string(slice.TypeTpu7x),
							"cloud.google.com/gke-tpu-partition-2x2x1-id": "f1",
						},
					},
					testingjobsjobset.ReplicatedJobRequirements{
						Name:        "rj2",
						...
						NodeSelector: map[string]string{
							"cloud.google.com/gke-tpu-accelerator":        string(slice.TypeTpu7x),
							"cloud.google.com/gke-tpu-partition-2x2x1-id": "f2",
						},
					},
				).
				RequestAndLimit("rj1", core.TPUResourceName, "1").
				RequestAndLimit("rj2", core.TPUResourceName, "1").
				Obj()
