...
var _ = ginkgo.BeforeSuite(func() {
	...
	for _, n := range nodes.Items {
		gomega.Eventually(func(g gomega.Gomega) {
			...
			partitionID := node.Labels["cloud.google.com/gke-tpu-partition-2x2x1-id"]
			...
			err := utilclient.PatchStatus(ctx, k8sClient, node, func() (bool, error) {
				node.Status.Capacity[core.TPUResourceName] = resource.MustParse(tpuCapacity)
				node.Status.Allocatable[core.TPUResourceName] = resource.MustParse(tpuCapacity)
				return true, nil
			})
