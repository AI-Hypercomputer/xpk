...
def test_cluster_create_calls_install_kueue_slice_controller_if_super_slicing(
    cluster_create_mocks: _ClusterCreateMocks, mocker
):
  args = construct_args(super_slicing=True)
  cluster_create_mocks.get_system_characteristics.return_value = (
      SUPER_SLICING_SYSTEM,
      0,
  )

  mocker.patch(
      'xpk.commands.cluster.should_validate_dependencies', return_value=False
  )
  mocker.patch('xpk.commands.cluster.add_zone_and_project')
  mocker.patch('xpk.commands.cluster._validate_cluster_create_args')
  mocker.patch(
      'xpk.commands.cluster.create_cluster_if_necessary', return_value=0
  )
  mocker.patch(
      'xpk.commands.cluster.authorize_private_cluster_access_if_necessary',
      return_value=0,
  )
  mocker.patch('xpk.commands.cluster.get_cluster_credentials')
  cluster_create_mocks.get_gke_control_plane_version.return_value = (0, '1.2.3')
  mocker.patch(
      'xpk.commands.cluster.update_coredns_if_necessary', return_value=0
  )
  mocker.patch('xpk.commands.cluster.install_storage_csis')
  mocker.patch(
  ...
  mocker.patch('xpk.commands.cluster.set_jobset_on_cluster', return_value=0)
  mocker.patch(
      'xpk.commands.cluster.update_jobset_resources_if_necessary',
      return_value=0,
  )
  mocker.patch('xpk.commands.cluster._install_kueue', return_value=0)
  mocker.patch('xpk.commands.cluster.prepare_gpus')
  mocker.patch(
      'xpk.commands.cluster.install_mldiagnostics_prerequisites', return_value=0
  )
  ...
  mock_install_slice_controller.assert_called_once()


def test_cluster_create_does_not_call_install_kueue_slice_controller_if_not_super_slicing(
    cluster_create_mocks: _ClusterCreateMocks, mocker
):
  args = construct_args(super_slicing=False)
  cluster_create_mocks.get_system_characteristics.return_value = (
      TPU_TEST_SYSTEM,
      0,
  )
  ...
  mock_install_slice_controller.assert_not_called()


def test_cluster_adapt_calls_install_kueue_slice_controller_if_super_slicing(
    cluster_create_mocks: _ClusterCreateMocks, mocker
):
  args = construct_args(super_slicing=True)
  cluster_create_mocks.get_system_characteristics.return_value = (
  ...
  mocker.patch('xpk.commands.cluster.is_dry_run', return_value=True)
