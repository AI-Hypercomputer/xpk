...
from xpk.commands.common import is_TAS_possible
...
class CommonCommandsTest(unittest.TestCase):
  ...
  def test_is_tas_possible_dry_run(
      self, mock_is_dry_run, mock_xpk_print, mock_xpk_exit, mock_run_command
  ):
    """Test is_TAS_possible returns True in dry_run mode."""
    mock_is_dry_run.return_value = True
    self.assertTrue(is_TAS_possible(None, None, "cluster", "zone", "project"))
  ...
  @patch("xpk.commands.common.xpk_exit")
  @patch("xpk.commands.common.xpk_print")
  def test_is_tas_possible_no_system_characteristics(
      self, mock_xpk_print, mock_xpk_exit, mock_is_dry_run
  ):
    """Test is_TAS_possible exits if system_characteristics is None."""
    mock_xpk_exit.side_effect = SystemExit(1)
    with self.assertRaises(SystemExit):
      is_TAS_possible(None, MagicMock(), "cluster", "zone", "project")
  ...
  @patch("xpk.commands.common.xpk_exit")
  @patch("xpk.commands.common.xpk_print")
  def test_is_tas_possible_no_capacity_type(
      self, mock_xpk_print, mock_xpk_exit, mock_is_dry_run
  ):
    """Test is_TAS_possible exits if capacity_type is None."""
    mock_xpk_exit.side_effect = SystemExit(1)
    with self.assertRaises(SystemExit):
      is_TAS_possible(MagicMock(), None, "cluster", "zone", "project")
    mock_xpk_print.assert_called_with(
  ...
  @patch("xpk.commands.common.is_dry_run", return_value=False)
  def test_is_tas_possible_h100_unsupported_capacity(self, mock_is_dry_run):
    """Test is_TAS_possible for H100 with unsupported capacity type."""
  ...
  @patch("xpk.commands.common.is_dry_run", return_value=False)
  def test_is_tas_possible_flex_start_capacity(self, mock_is_dry_run):
    """Test is_TAS_possible returns True for FLEX_START capacity."""
    mock_system = MagicMock(spec=SystemCharacteristics)
  ...
  @patch("xpk.commands.common.run_command_for_value")
  @patch("xpk.commands.common.is_dry_run", return_value=False)
  def test_is_tas_possible_compact_placement_exists(
      self, mock_is_dry_run, mock_run_command
  ):
    """Test is_TAS_possible with COMPACT placement returns True."""
    mock_system = MagicMock(spec=SystemCharacteristics)
  ...
  @patch("xpk.commands.common.run_command_for_value")
  @patch("xpk.commands.common.is_dry_run", return_value=False)
  def test_is_tas_possible_no_compact_placement(
      self, mock_is_dry_run, mock_run_command
  ):
    """Test is_TAS_possible without COMPACT placement returns False."""
    mock_system = MagicMock(spec=SystemCharacteristics)
    mock_system.device_type = "a3-ultra"
  ...
  @patch("xpk.commands.common.run_command_for_value")
  @patch("xpk.commands.common.is_dry_run", return_value=False)
  def test_is_tas_possible_command_fails(
      self, mock_is_dry_run, mock_run_command, mock_xpk_print
  ):
    """Test is_TAS_possible when gcloud command fails."""
    mock_system = MagicMock(spec=SystemCharacteristics)
    mock_system.device_type = "a3-ultra"
