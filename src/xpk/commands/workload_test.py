...
def test_workload_create_pathways_jobset_yaml(mocker):
    ...
    mocker.patch('xpk.commands.workload.get_system_characteristics', return_value=(workload_system, 0))
    mocker.patch('xpk.commands.workload.get_cluster_configmap', return_value=None)
    mocker.patch('xpk.commands.workload.check_if_workload_can_schedule', return_value=True)
    mocker.patch('xpk.commands.workload.is_autoprovisioning_enabled', return_value=(False, 0))
    mocker.patch('xpk.commands.workload.check_if_workload_exists', return_value=False)
    mocker.patch('xpk.commands.workload.ensure_pathways_workload_prerequisites', return_value=True)
    mocker.patch('xpk.core.pathways.get_user_workload_container', return_value=("- name: test-docker\n  image: test-image", "123"))
    mocker.patch('xpk.commands.workload.create_tpu_topology', return_value="4x4")
    mocker.patch('xpk.commands.workload.create_tpu_machine_type', return_value="ct4p-hightpu-4t")

    mock_write_file = mocker.patch('builtins.open', mocker.mock_open())
    
    mocker.patch('xpk.commands.workload.write_tmp_file', return_value="/tmp/test.yaml")
    ...
    mocker.patch('xpk.commands.workload.get_gke_outlier_dashboard', return_value=None)

    workload_create(args)

    mock_write_file.assert_called_once_with('pw_manifest.yaml', 'w', encoding='utf-8')
