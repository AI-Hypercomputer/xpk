...
@mock.patch("xpk.core.cluster_toolkit.native_manager.run_command_with_full_controls")
...
def test_run_command_failure(mock_run, mock_print, mock_exit, runner):
    ...
    mock_print.assert_any_call("Running gcluster command: echo hello failed with code 123.")
...
@mock.patch("xpk.core.cluster_toolkit.native_manager.xpk_print")
def test_upload_directory_to_working_dir(mock_print, mock_ensure, mock_copytree, runner):
    result = runner.upload_directory_to_working_dir("/path/to/my_dir", prefix="pre")
    ...
    mock_copytree.assert_called_once_with("/path/to/my_dir", expected_target_path, dirs_exist_ok=True)
    mock_print.assert_any_call(f"Copying directory from /path/to/my_dir to {expected_target_path}.")
    assert result == expected_target_path

@mock.patch("xpk.core.cluster_toolkit.native_manager.copytree")
@mock.patch("xpk.core.cluster_toolkit.native_manager.ensure_directory_exists")
def test_upload_directory_to_working_dir_no_prefix(mock_ensure, mock_copytree, runner):
...
def test_upload_file_to_working_dir(mock_print, mock_ensure, mock_copy, runner):
    result = runner.upload_file_to_working_dir("/path/to/my_file.txt", prefix="pre")

    expected_upload_dir = os.path.join("/fake/working_dir", "uploads", "pre")
    expected_target_path = os.path.join(expected_upload_dir, "my_file.txt")

    mock_ensure.assert_called_once_with(expected_upload_dir)
    mock_copy.assert_called_once_with("/path/to/my_file.txt", expected_target_path)
    mock_print.assert_any_call(f"Copying a file from /path/to/my_file.txt to {expected_target_path}.")
    assert result == expected_target_path
