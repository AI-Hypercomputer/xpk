...
class NodePoolManager:
  ...
  def run_gke_node_pool_create_command(self, gke_node_pool_version) -> int:
    ...
    existing_node_pool_names, return_code = (
        self.get_all_nodepools_programmatic()
    )
    if return_code > 0:
    ...
    if return_code > 0:
      ...
      return return_code
    if capacity_type == CapacityType.UNKNOWN.value:
    ...
    if existing_node_pool_names:
      ...
      for node_pool_name in existing_node_pool_names:
        if node_pool_name.find(f'{self.args.cluster}-np-') != 0:
          continue
      ...
      if (
          self.args.enable_workload_identity
          or self.args.enable_gcsfuse_csi_driver
      ):
        for node_pool_name in existing_node_pool_names:
          if not node_pool_name in node_pools_to_delete:
            ...
            return_code, existing_node_pool_medadata_mode = (
                self.get_nodepool_workload_metadata_mode(node_pool_name)
            )
            if return_code != 0:
              return 1
            ...
            if (
              ...
              command = (
                  ...
                  f' {node_pool_name} --cluster={self.args.cluster} --zone={GCloudContextManager.zone_to_region(self.args.zone)} --project={self.args.project} --quiet'
    ...
    if update_WI_commands:
      will_update_WI = True
      if node_pools_to_update_WI and not self.args.force:
      ...
      if not will_update_WI:
        for i, command in enumerate(update_WI_commands):
          xpk_print(
              f'To complete {update_WI_task_names[i]} we are executing'
              f' {command}'
          )
        max_return_code = run_commands(
            update_WI_commands,
      ...
      if not node_pools_to_remain:
        ...
        resources_configmap_name = f'{self.args.cluster}-{self.resource_manager.CLUSTER_RESOURCES_CONFIGMAP}'
        resources_yml = self.resource_manager.CLUSTER_CONFIGMAP_YAML.format(
            args=self.args, name=resources_configmap_name, data=resources_data
        )
        configmap_yml = {}
    ...
    for node_pool_name in desired_node_pool_names:
      ...
      if self.system.accelerator_type == AcceleratorType['TPU']:
        ...
        command += f' --num-nodes={self.system.vms_per_slice}'
        command += ' --placement-type=COMPACT  --max-pods-per-node 15'
        command += (
            f' --scopes=storage-full,gke-default,{CLOUD_PLATFORM_AUTH_SCOPE_URL}'
        )
        command += f' --tpu-topology={self.system.topology}'
        command += f' {self.args.custom_tpu_nodepool_arguments}'
        ...
        command += f' --num-nodes={self.args.num_nodes}'
        command += (
            ' --accelerator'
        ...
        if device_type == DeviceType.H100_MEGA.value:
          command += (
              ' --additional-node-network'
  ...
  def get_node_pools_to_delete(
    ...
    check_resource, is_requested_resource_in_cluster = (
        self.resource_manager.check_cluster_resources()
    )
  ...
  def get_all_nodepools_programmatic(self) -> tuple[list[str], int]:
  ...
  def upgrade_gke_nodepools_version(self, default_rapid_gke_version) -> int:
  ...
  def get_nodepool_workload_metadata_mode(
    ...
    command = (
        f'gcloud beta container node-pools describe {nodepool_name}'
        f' --cluster {self.args.cluster} --project={self.args.project}'
        f' --region={GCloudContextManager.zone_to_region(self.args.zone)} --format="value(config.workloadMetadataConfig.mode)"'
    )
    return_code, nodepool_WI_mode = run_command_for_value(
        command, 'Get Node Pool Workload Identity Metadata Mode', self.args
    )
    if return_code != 0:
      xpk_print(
