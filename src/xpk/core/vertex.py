...
class VertexAI:
  ...
  def create_vertex_tensorboard(self) -> dict:
    ...
    tensorboard_name = self.args.tensorboard_name or f'{self.args.cluster}-{DEFAULT_VERTEX_TENSORBOARD_NAME}'
  ...
  def create_vertex_experiment(self) -> dict | None:
    ...
    metadata_configmap_name = f'{self.args.cluster}-{CLUSTER_METADATA_CONFIGMAP}'
    cluster_config_map = get_cluster_configmap(self.args, metadata_configmap_name)
    ...
    tensorboard_config = {
      ...
      'experiment_name': self.args.experiment_name or f'{self.args.cluster}-{self.args.workload}'
    }
