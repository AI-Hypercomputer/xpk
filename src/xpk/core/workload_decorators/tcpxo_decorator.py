...
def decorate_jobset_with_tcpxo(jobset_manifest_str, additional_networks) -> str:
  """
  ...
  for job in manifest['spec']['replicatedJobs']:
    ...
    add_annotations(job_manifest, additional_networks)
...
def add_annotations(job_manifest, additional_networks):
  ...
  interfaces = [
      ...
      *[
          f'    {{"interfaceName":"eth{i + 1}","network":"{additional_networks[i]}"}}{"," if i<7 else ""}'
          for i in range(8)
      ],
      ']',
  ]
