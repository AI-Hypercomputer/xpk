...
def build_script(blocks: list[CodeBlock], mode: Mode) -> str:
    ...
    for i, block in enumerate(blocks):
        ...
        cmd_display = f"'$ {safe_command}'"

        script_parts.append(f"echo 'XPK_RECIPE_EXECUTOR_BLOCK_START'")
        script_parts.append(f"echo {cmd_display}")
        script_parts.append(f"{block.command}")
        script_parts.append(f"echo 'XPK_RECIPE_EXECUTOR_BLOCK_END'")
...
def extract_script_outputs(output: str) -> tuple[list[str], bool]:
    ...
    start_marker = f"XPK_RECIPE_EXECUTOR_BLOCK_START"
    end_marker = f"XPK_RECIPE_EXECUTOR_BLOCK_END"
...
def reconstruct_content(content: str, blocks: list[CodeBlock], results: list[str]) -> str:
    last_pos = 0
    new_content_chunks = []
...
def process_file(filepath: str, mode: Mode):
    ...
    if mode == Mode.UPDATE:
        print(f"{Color.YELLOW}Updating: {filepath}...{Color.NC} ", end='', flush=True)
    elif mode == Mode.RUN:
        print(f"{Color.YELLOW}Running: {filepath}...{Color.NC} ", end='', flush=True)
    elif mode == Mode.GOLDEN:
        print(f"{Color.YELLOW}Verifying: {filepath}...{Color.NC} ", end='', flush=True)
